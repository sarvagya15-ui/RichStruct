...
def select_schema(): 
    schema_types = [
        {'id': 'product', 'name': 'Product', 'desc': 'Rich snippets for E-commerce.', 'icon': 'fa-shopping-cart'}, 
        {'id': 'faq', 'name': 'FAQ', 'desc': 'Questions and answers.', 'icon': 'fa-question-circle'}, 
        {'id': 'course', 'name': 'Course', 'desc': 'Educational content.', 'icon': 'fa-graduation-cap'} 
    ] 
...
def generate_jsonld(schema_type): 
    ...
    new_schema = Schema(schema_type=schema_type, schema_name=name_for_storage, 
    ...
    db.session.commit() 
    return render_template('result.html', json_data=json_string, schema_type=schema_type) 
...
def validate(): 
    ...
    if request.method == 'POST': 
        ...
        try: 
            ...
            validation_report = { 'status': 'error' if errors else ('warning' if warnings else 'success'), 
                'message': 'Compliance Report', 'details': f"Found {len(errors)} errors and {len(warnings)} tips.", 'errors': errors, 'warnings': warnings, 'score': score, 
